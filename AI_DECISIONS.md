...
3. **Retry/backoff, but only when retrying can help**: transient per-minute 429s are retried with exponential backoff (4s/8s/16s). Daily-quota-exhaustion 429s (`"tokens per day"` / `"TPD"` / `"RPD"` in the error) are detected separately and fail fast instead — retrying an exhausted daily cap within the same call can never succeed, so backing off for 28s per message would just be wasted latency, discovered and fixed after hitting this exact case live during development (see below).
